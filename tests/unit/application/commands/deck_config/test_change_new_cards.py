...
from tests.unit.factories.values import create_new_cards_config
...
async def test_changes_new_cards_successfully(
    ...
    handler = ChangeNewCardsCommandHandler(
    ...
    # Act
    await handler(ChangeNewCardsCommand(deck_config_id=uuid4(), new_cards=new_cards))

    # Assert
    assert config.new_cards == new_cards
