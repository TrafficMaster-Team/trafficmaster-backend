...
def _handler(cus: Mock, us: Mock, acl: Mock, gw: Mock, tx: Mock) -> RevokeAdminByIdCommandHandler:
    return RevokeAdminByIdCommandHandler(
...
async def test_revokes_admin_successfully(
    ...
    # Act
    await handler(RevokeAdminByIdCommand(user_id=uuid4()))

    # Assert
    fake_access_service.toggle_admin_role.assert_called_once_with(target, is_admin=False)
