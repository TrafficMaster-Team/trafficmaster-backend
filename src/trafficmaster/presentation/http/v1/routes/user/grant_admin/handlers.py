...
grant_admin_route: Final[APIRouter] = APIRouter(
...
@grant_admin_route.patch(
    "/{user_id}/admin",
    status_code=status.HTTP_204_NO_CONTENT,
    summary="Grant admin role",
    description=getdoc(GrantAdminByIdCommandHandler),
...
async def grant_admin_by_id(
    user_id: Annotated[UUID, UserIDPathParameter],
    interactor: FromDishka[GrantAdminByIdCommandHandler],
) -> None:
    command: GrantAdminByIdCommand = GrantAdminByIdCommand(user_id=user_id)
