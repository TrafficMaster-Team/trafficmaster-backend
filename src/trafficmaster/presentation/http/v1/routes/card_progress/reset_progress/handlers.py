...
reset_progress_route: Final[APIRouter] = APIRouter(tags=["Card Progress"], route_class=DishkaRoute)


CardIDPathParameter = Path(
    title="The ID of the card",
    description="The ID of the card whose progress to reset. We using UUID id's",
    examples=["19178bf6-8f84-406e-b213-102ec84fab9f"],
)


@reset_progress_route.post(
    "/card/{card_id}/reset",
    status_code=status.HTTP_204_NO_CONTENT,
    summary="Reset a card's progress",
    description=getdoc(ResetCardProgressCommandHandler),
...
async def reset_card_progress(
    ...
    interactor: FromDishka[ResetCardProgressCommandHandler],
) -> None:
    command: ResetCardProgressCommand = ResetCardProgressCommand(card_id=card_id)
