...
class DeleteDeckConfigCommand:
    deck_config_id: UUID


class DeleteDeckConfigCommandHandler:
    def __init__(
        self,
    ...
    async def __call__(self, data: DeleteDeckConfigCommand) -> None:
        ...
        if await self._deck_gateway.exists_with_deck_config_id(deck_config.id):
            msg = "Deck config is in use by one or more decks"
            raise DeckConfigInUseError(msg)

        await self._deck_config_gateway.delete_by_id(deck_config.id)
