...
class GrantAdminByIdCommand:
    user_id: UUID


class GrantAdminByIdCommandHandler:
    def __init__(
        self,
    ...
    async def __call__(self, data: GrantAdminByIdCommand) -> None:

        current_user = await self._current_user_service.get_current_user()

        user_for_granting: User | None = await self._user_gateway.read_by_id(UserID(data.user_id))

        if user_for_granting is None:
            msg = f"Can't find user with id {data.user_id}"
            raise UserNotFoundByIdError(msg)

        if not self._access_service.can_manage_user(current_user, user_for_granting):
            msg = "You don't have permission to do that"
            raise NoPermissionToManageUserError(msg)

        self._access_service.toggle_admin_role(user_for_granting, is_admin=True)
