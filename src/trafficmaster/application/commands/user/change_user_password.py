...
class ChangeUserPasswordCommandHandler:
    ...
    async def __call__(self, data: ChangeUserPasswordCommand) -> None:
        ...
        user_for_update_password: User | None = await self._user_gateway.read_by_id(UserID(data.user_id))

        if user_for_update_password is None:
            msg = f"Can't find user with id {data.user_id}"
            raise UserNotFoundByIdError(msg)

        if not self._access_service.can_manage_user(user_for_update_password, current_user):
