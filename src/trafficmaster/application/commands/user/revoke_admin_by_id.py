...
class RevokeAdminByIdCommandHandler:
    ...
    async def __call__(self, data: RevokeAdminByIdCommand) -> None:
        ...
        user_for_revoking: User | None = await self._user_gateway.read_by_id(UserID(data.user_id))

        if user_for_revoking is None:
            msg = f"Can't find user with id {data.user_id}"
            raise UserNotFoundByIdError(msg)

        if not self._access_service.can_manage_user(current_user, user_for_revoking):
            msg = "You don't have permission to do that"
            raise NoPermissionToManageUserError(msg)

        self._access_service.toggle_admin_role(user_for_revoking, is_admin=False)
